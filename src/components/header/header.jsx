...
export default function Header() {
  ...
  return (
    <header className="z-[999] relative">
      <div
        id="nav"
        className="fixed text-base flex flex-col justify-center items-center bg-bkg-2/60 border border-text/10 h-[7.25rem] inset-0 my-0 mx-auto shadow-lg shadow-black/[0.1] md:w-[60rem] md:h-[5.5rem] md:rounded-full md:top-5 2xl:w-[80rem]  overflow-hidden"
      >
        <nav className="h-12 flex fixed left-1/2 -translate-x-1/2 sm:h-[initial] sm:rounded-full">
          ...
          <div className="relative flex items-center overflow-hidden user-select-none">
            <img
              src={catlogo}
              alt="logo"
              className="logo m-1 float-left h-12 w-12 sm:h-20 sm:w-20 transition-transform duration-[1000ms] ease-in-out sm:display-none"
              style={{
                position: "relative",
                display: "flex",
                userSelect: "none",
